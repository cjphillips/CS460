...
int fork()
{
  ...
  for (i = 0; i < NFD; i++)
  {
    ...
    if (p->fd[i] != 0)
    {
      ...
      {
        p->fd[i]->ppipe->nReader++;
      }
      if (p->fd[i]->mode == WRITE_PIPE)
      {
        p->fd[i]->ppipe->nWriter++;
      }
    }
  }
...
int kexit(int exitValue)
{
  ...
  {
    if (exitValue == BROKEN_PIPE)
    {
      printf("[KERNEL] Attempting to exit due to a broken pipe.\n");
    }
  ...
  {
    case BROKEN_PIPE:
      printf("[KERNEL] Broken pipe. Exiting process [%d].", running->pid);
      break;
    default:
